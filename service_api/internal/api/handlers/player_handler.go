...
func (h *PlayerHandler) RegisterRoutes(router *gin.RouterGroup, authMiddleware gin.HandlerFunc, serverToServerAuthMiddleware gin.HandlerFunc) {
	player := router.Group("/players")
	{
		...
		player.PUT("/chip", serverToServerAuthMiddleware, h.IncrementChip)
...
func (h *PlayerHandler) IncrementChip(c *gin.Context) {
	model := BindModel[IncrementChipRequest](c)
	if model == nil {
		return
	}
...
// IncrementChipRequest represents a request to increment a player's chips
